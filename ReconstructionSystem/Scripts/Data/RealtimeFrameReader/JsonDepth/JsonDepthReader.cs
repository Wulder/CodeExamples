...
public class JsonDepthReader : FrameReader
{
    ...
    void ReadPoses(out Vector3 position, out Quaternion rotation)
    {
        ...
        rotation = Quaternion.EulerRotation(rotX, rotY, rotZ);
    ...
    uint[] ReadDepthImage(string path)
    {
        ...
        for(int i = 0; i < 192; i++)
        {
            for(int j = 0; j < 256; j++)
            {
                depth[i*256 + j] = Math.Clamp((uint)((float.Parse(data["depth_data"][i][j].ToString())/65f) * 65535f),0,UInt16.MaxValue);
            }
        }
